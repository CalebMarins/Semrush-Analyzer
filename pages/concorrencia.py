...
clcf = {
        'Trends': st.column_config.BarChartColumn(
            y_min=0,import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
import plotly.express as px
import tldextract
...
if 'data' in st.session_state:
    ...
    def kw_detail(item):
        ...
        st.markdown(f'##### Detalhes sobre termo: :blue[{termo_selecionado['Keyword'].iloc[0]}]')
        c1,c2 = st.columns(2)
        with c1:
            c111,c222 = st.columns(2)
            with c111:
                st.markdown(f'**Palavra chave:** :blue-badge[{termo_selecionado['Keyword'].iloc[0]}]')
            with c222:
                st.markdown(f'Volume de busca: :blue-badge[{termo_selecionado['Search Volume'].iloc[0]}]')
        c1_1,c1_2=st.columns([1,3])
        with c1_1:
            st.metric(label=f'Posição {cliente}', value=termo_selecionado['Position'].min(), delta_color="inverse", border=True)
            if selec =='Todos':
                xx=list(iinter['Conc'].unique())
                cli= f'Posição {cliente}'
                j={}
                j[cli] = termo_selecionado['Position'].min()
                for i in xx:
                    st.metric(label=f'Posição {i}', value=termo_selecionado[f'Posição {i}'].min(),format='plain', delta_color="inverse", border=True,)
                    pos = f'Posição {i}'
                    if termo_selecionado[pos].min() >0:
                        j[pos]=termo_selecionado[pos].min()
                    else:
                        j[pos]=100
            else:
                ...
                j[pos] = termo_selecionado[pos].min()
        with c1_2:
            jj = pd.Series(j, name='Posição').to_frame()

            valor =100
            jj.columns = ['Posição']
            jj['Valor gráfico'] = valor - jj['Posição']
            jj['Nome_Exibicao'] = (jj['Posição'].astype(int)).astype(str)
            jj.loc[jj['Nome_Exibicao'] == '100', 'Nome_Exibicao'] = '-'
            ...
            for i in jj.index:
                if cont==1:
                    map_color[i]= '#000050'
                else:map_color[i]= '#9CF026'
                cont= cont+1

            fig = px.bar(jj, x=jj.index, y='Valor gráfico', text='Nome_Exibicao',color=jj.index,color_discrete_map =map_color)             
            fig.update_layout(showlegend=False, hovermode=False, font=dict(size=14))
            fig.update_yaxes(showgrid=False,showticklabels=False, zeroline=False, title_text="")
            fig.update_traces(hoverinfo='none', hovertemplate=None, marker_cornerradius=15, cliponaxis=False, textposition="outside")
    ...
    if conc_file:
        ...
        for conc_file in conc_file:
            ...
            #--------------------ESTILIZAÇÃO DE URL CONCORRRENTE--------------------#
            xx=list(inter['Conc'].unique())
            j={}
            for i in xx:
                nome = f'URL {i}'
                j[nome]=st.column_config.LinkColumn()
            clcf|=j
        ...
        if resumo:
            ...
            if selec=='Todos':
                for i in df_melt['Concorrente'].unique():
                    ...
                    c1.metric(f'Keywords em comum - {i}',x, border=True, format='localized')
                    x=df_melt[(df_melt['Concorrente']==i)&(df_melt['Tipo']=='Exclusivas concorrente')]['Quantidade']
                    c2.metric(f'Exclusivas concorrente - {i}',x, border=True, format='localized')
                    x=df_melt[(df_melt['Concorrente']==i)&(df_melt['Tipo']=='Exclusivas cliente')]['Quantidade']
                    c3.metric(f'Exclusivas {cliente}',x, border=True, format='localized')
                    plot_sel=df_melt 
            else:
                st.markdown(f'##### {selec}')
                ...
                c1.metric(f'Keywords em comum - {selec}',x, border=True, format='localized')
                x=df_melt[(df_melt['Concorrente']==selec)&(df_melt['Tipo']=='Exclusivas concorrente')]['Quantidade']
                c2.metric(f'Exclusivas concorrente - {selec}',x, border=True, format='localized')
        ...
        if detail:
            ...
            if selec == 'Todos':
                for i in list(dict_concorrentes.keys()):
                    ...
                    st.metric(f'{textinho} :blue[**{i}**] nas posições :blue[**{i_pos} a {f_pos}**]',x, border=True, format='localized')
            else:
                x=len(iinter[(iinter['Conc']==selec)&(filtro_pos_inter)]['Keyword'].unique())
                st.metric(f'{textinho} {selec} nas posições {i_pos,f_pos}',x, border=True, format='localized')
            st.divider()
        ...
        if insights:
            ...
            for i in dict_concorrentes.keys():
                ...
                st.metric(f'{texto_metric} :blue[**{i}**] nas posições :blue[**{i_pos} a {f_pos}**]',x, border=True, format='localized')
            y_max=100,
        ),
        'Google':st.column_config.LinkColumn(display_text="Abrir no Google"),
        'URL':st.column_config.LinkColumn(),
